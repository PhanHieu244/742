...
public class CharacterScroller : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < CharacterManager.Instance.characters.Length; i++)
        {
            ...
            if (charData.IsUnlocked || charData.isFree)
                ChangeLockColor(character, Color.white);
            else
                ChangeLockColor(character, lockColor);
            ...
            switch (scrollerStyle)
            {
                ...
                case ScrollerStyle.Circle:
                    ...
                    float d = Mathf.Abs(Vector3.Angle(Vector3.back, (character.transform.localPosition - transform.InverseTransformPoint(centerPoint)).normalized) * Mathf.Deg2Rad);
                    if (d > GameManager.Instance.hideKnifeTextureAngle)
                        ChangeLockColor(character, lockColor);
                    else if (character.GetComponent<Character>().IsUnlocked)
                        ChangeLockColor(character, Color.white);
                    break;
                default:
                    break;
    ...
    void MoveAndScaleCircleVer(Transform tf, Vector3 moveVector, int index)
    {
        ...
        if (d> GameManager.Instance.hideKnifeTextureAngle)
        {
            ChangeLockColor(tf.gameObject, lockColor);
        }
        else if(tf.GetComponent<Character>().IsUnlocked)
        {
            ChangeLockColor(tf.gameObject, Color.white);
        }
    }
    ...
    public void UnlockCharacter()
    {
        ...
        if (unlockSucceeded)
        {
            ...
            ChangeLockColor(currentCharacter, Color.white);
            unlockButton.gameObject.SetActive(false);
            selectButon.gameObject.SetActive(true);
    ...
    private void ChangeLockColor(GameObject currentCharacter, Color color)
    {
        MeshRenderer[] meshRendererList = currentCharacter.GetComponentsInChildren<MeshRenderer>();
